...
namespace CodeContracts.Fody.Tests.ContractScanners
{
    public class PropertyScannerTests
    {
        ...
        [Theory(DisplayName = "Проверка получения атрибута контракта инварианта для метода получения свойства"), AutoFixture]
        public void ScanContractGetInvariantTest(
            [Frozen]ModuleDefinition moduleDefinition,
            [Frozen]Mock<IContractCriteria> contractCriteriaMock,
            PropertyScanner sut)
        {
            ...
            var contracts = sut.Scan(moduleDefinition.FindProperty("DarthPlagueis", "Name")).ToList();

            contractCriteriaMock.Verify(cc => cc.IsContract(It.IsAny<CustomAttribute>()));
            Assert.Single(contracts.OfType<InvariantDefinition>());
        }

        [Theory(DisplayName = "Проверка отсутствия атрибута контракта предусловия для метода получения свойства"), AutoFixture]
        public void ScanNoContractGetInvariantTest(
            [Frozen]ModuleDefinition moduleDefinition,
            [Frozen]Mock<IContractCriteria> contractCriteriaMock,
            PropertyScanner sut)
        {
